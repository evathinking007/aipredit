...
class AIPredict:
    ...
    def write_next_data(self, bitcoin_data, future_price):
        bitcoin_data.loc[len(bitcoin_data), 'Future_Price']= future_price
        #bitcoin_data['Future_Price'].iloc[0] 
...
class GateIO_Api:
    ...
    def get_current_data_api(self,file_path):
        ...
        for i in r.json():
            ...
            # bitcoin_data.loc[len(bitcoin_data)-1, 'Date']= data_k_date
            # bitcoin_data.loc[len(bitcoin_data)-1, 'Price']= float(data_k_price)
            # bitcoin_data.loc[len(bitcoin_data)-1, 'Volume']= float(data_k_volume)
            bitcoin_data.to_excel(file_path, index=False)
