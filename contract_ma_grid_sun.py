...
class Contract:
    ...
    def execute_trade(self, action, quantity, price):
        ...
        if action == "buy" and cost <= self.balance:
            # 调用买入合约接口
            self.submit_order(quantity,price)
    ...
    def get_balance(self):
        ...
        self.balance=futures_account.total
...
class BackTest:
    def __init__(self, logger,coin_name,start_time,end_time,period='5m'):
        ...
        self.start_time=str(datetime.strptime(start_time, self.dt_format).timestamp()).replace('.0', '')
        self.end_time=str(datetime.strptime(end_time, self.dt_format).timestamp()).replace('.0', '')
        self.period=period
    ...
    def get_history_data(self):
        ...
        sheet['D1'] = 'Low'
        sheet['E1'] = 'Close'
        file_path = self.coin_name + "_"+str(self.start_time)+"_history_data.xlsx"
        wb.save(file_path)
        ...
        headers = {'Accept': 'application/json', 'Content-Type': 'application/json'}
        url = '/futures/usdt/candlesticks'
        query_param = 'contract=' + self.coin_name + '_USDT&from='+str(self.start_time)+'&to='+str(self.end_time)+'&interval='+self.period
        # query_param = 'currency_pair=' + self.coin_name + '_USDT&limit=1000&interval='+self.period
        # query_param = 'currency_pair=' + self.coin_name + '_USDT&from='+str(self.start_time)+'&to='+str(self.end_time)+'&interval='+self.period
    ...
    def get_1day_base_price(self,time_point):
        # Convert string to datetime object
        dt_format = "%Y-%m-%d %H:%M:%S"
        dt_obj = datetime.strptime(time_point, dt_format).replace(hour=8, minute=0, second=0, microsecond=0)


        # Subtract one day
        one_day_before = dt_obj - timedelta(days=1)
        one_day_before_str=str(one_day_before)
        workbook = openpyxl.load_workbook(self.coin_name + "_history_data_1day.xlsx")
        # 选择第一个工作表
        worksheet = workbook.active
        max_row=worksheet.max_row
        for i in range(2,max_row+1):
            if str(worksheet.cell(row=i, column=1).value)==one_day_before_str:
                return one_day_before_str,worksheet.cell(row=i, column=5).value

        # # Format back to string
        # back_1day=one_day_before.timestamp()
        # # end_day=dt_obj.timestamp()
        # host = "https://api.gateio.ws"
        # prefix = "/api/v4"
        # headers = {'Accept': 'application/json', 'Content-Type': 'application/json'}
        ...
        # query_param = 'currency_pair=' + self.coin_name + '_USDT&from='+str(back_1day).replace('.0', '')+'&to='+str(back_1day).replace('.0', '')+'&interval=1d'
        # # print(query_param)
        # r = requests.request('GET', host + prefix + url + "?" + query_param, headers=headers)
        # print(r.json())
        # return r.json()[0][5]
    ...
    def caculate_ma(self,history_file_path):
        df = pd.read_excel(history_file_path)
        ma_period = 20  # 移动平均的周期
        df['MA'] = df['Close'].rolling(window=ma_period).mean()
...
def run_back_test(coin_name,start_time,end_time,period,initial_balance=1000,investment_per_trade=100,grid_start=1,grid_step=0.5,sell_step=1.025,buy_step=0.985):
    minutes_to_add = 5

    # 创建日志把手
    log_file_name = f"{coin_name}_execute.log"
    ...
    for i in range(2,max_row):
         # 得到当前时间点
        now_time=str(worksheet.cell(row=i, column=1).value)

        # 得到当前价格
        current_price=worksheet.cell(row=i, column=5).value
        logger.info("当前的价格是："+str(current_price)+",时间是:"+str(now_time))

        # 得到对比均线价格
        base_price = float(worksheet.cell(row=i, column=6).value)
        logger.info("当前均线价格是："+str(base_price))
        ...
        if len(sell_trades) < 5:
            ...
            logger.info("当前的价格低于均线："+str(buy_percentage))
        ...
        if len(buy_trades) < 5:
            ...
            logger.info("做空，当前的价格高于均线："+str(sell_percentage))
        ...
        # time_point=str(now+ timedelta(minutes=5))
        # logger.info("下一次的时间时:"+time_point)

def trading_run(coin_name,period,initial_balance=1000,investment_per_trade=100,grid_start=1,grid_step=0.5,sell_step=1.025,buy_step=0.975):
